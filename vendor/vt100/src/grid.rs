...
impl Grid {
    ...
    pub fn set_size(&mut self, size: Size) {
        // Keep the live cursor visible when shrinking, retaining displaced
        // rows in history. Resizing must not discard the latest serial output.
        // Work on cells directly so incomplete UTF-8/escape input stays intact.
        if !self.rows.is_empty() && size.rows < self.size.rows {
            let removed = self
                .pos
                .row
                .saturating_add(1)
                .saturating_sub(size.rows)
                .min(self.size.rows - size.rows);
            for _ in 0..removed {
                let row = self.rows.remove(0);
                self.push_scrollback(row);
            }
            self.pos.row = self.pos.row.saturating_sub(removed);
            self.saved_pos.row = self.saved_pos.row.saturating_sub(removed);
        } else if !self.rows.is_empty() && size.rows > self.size.rows {
            let mut restored = 0;
            for _ in 0..(size.rows - self.size.rows) {
            ...
            self.pos.row += restored;
            self.saved_pos.row += restored;
            self.scrollback_offset =
                self.scrollback_offset.saturating_sub(usize::from(restored));
        }
